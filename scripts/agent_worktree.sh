...
"$repo_root/scripts/install_agent_guard.sh"

# Ownership is worktree state. Never write it to the shared repository config.
git -C "$dir" config --worktree --unset-all calblue.readOnly 2>/dev/null || true
git -C "$dir" config --worktree calblue.agent "$agent"
git -C "$dir" config --worktree calblue.sessionId "$agent-$(date +%Y%m%d-%H%M%S)"
git -C "$dir" config --worktree calblue.allowedBranchRegex "^(chore|feat|fix)/.*-$agent\$"
