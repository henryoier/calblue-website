...
The deploy workflow runs both syncs every six hours and before every Pages deployment. It writes small `data/swpl.json` and `data/nccsf.json` snapshots. The homepage merges both feeds chronologically and emphasizes the next five matches; dedicated SWPL and NCCSF pages show each complete season schedule. Until SWPL publishes its full season, `data/swpl-overrides.json` supplies the league fixtures from the September 3 preview and two Abronzino Cup Group C fixtures from the schedule supplied September 14 (`4a07243c9bceb3e7e44759b7b0abf080.jpg`). The Cup games are October 25 at 7:00 PM away to JSC JASA at Red Morton Park - Bechet Field, and November 22 at 7:30 PM home to South San Francisco AC at Fair Oaks Park Field 3. Times are interpreted as Pacific Time, with the correct daylight-saving offsets. The former September 26 Cup placeholder was removed because no CalBlue game is listed that weekend. Official SWPL rows automatically replace matching preview entries within the same competition, even when a fixture's date changes; a league game never replaces a Cup game against the same opponent. The NCCSF importer also reads the official team directory so every club uses its published crest, including the site's mixed `.png`, `.jpeg`, and `.jpg` filenames. The importers only accept rows involving CalBlue and deliberately ignore contact, player, and unrelated-team data. If either official source is unavailable or changes structure, deployment stops and the previous Pages deployment remains live.
